 ...
 *
 * <p>挂载于 {@code /internal/**} 前缀，网关不为该前缀定义外部路由，
 * 避免凭据校验这类接口被外部直连调用
 *
...
public class SysUserInternalController {
    ...
    /**
     * 凭据校验：校验账号密码，通过则返回登录身份包
     *
     * @param dto 账号 + 密码，两者均不能为空
     * @return 登录身份包（用户信息 + 角色编码 + 权限标识）；账号不存在、歧义或密码错误抛异常
     * @author yeungzhy
     * @since 2026-08-09
     */
    @PostMapping("/verify")
     ...
     *
     * @param dto 用户 ID，不能为空
     * @return 已建树的菜单树节点，仅目录/菜单、不含按钮；超管返回全量
     * @author yeungzhy
     * @since 2026-08-15
     */
     ...
     * 故返回本页数据本身、不带 total，没有可信的 total 就不该造一个恒为 0 的字段出来
     *
     * @param dto 查询条件 + 分页参数（pageNum / pageSize）；查询条件须与 exportTotal 保持一致，
     *            否则翻页过程中取数口径会漂移
     * @return 当前页用户数据，空列表表示已取完，不为 null
     * @author yeungzhy
     * @since 2026-09-07
     */
