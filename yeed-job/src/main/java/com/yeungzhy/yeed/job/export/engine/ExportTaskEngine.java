 ...
 * 本类是失败链的终点，装配点无需感知引擎成败，也无需重复回写。重试与告警属调度侧未来能力（崩溃自愈），
 * 引擎内不处理
 *
 * <p> 注意，本语义只覆盖引擎管线：{@link ExportContext} 的装配（实体加载、JSON → 参数反序列化）发生在
...
public class ExportTaskEngine {
     ...
     * 任一环节失败则先记录日志、回写失败原因，再回调 {@link Exporter#onError} 清理资源（清理失败被隔离、止于日志），
    ...
    public <P, R> void execute(ExportContext<P> ctx, Exporter<P, R> exporter) {
        ...
        try {
            ...
            log.info("导出任务取总行数完成，taskId={}，总行数={}，耗时={}ms",
                    ctx.taskId(), total, stopWatch.lastTaskInfo().getTimeMillis());
            ...
            log.info("导出任务写文件完成，taskId={}，总行数={}，字节={}，耗时={}ms",
                    ctx.taskId(), total, excelData.length, stopWatch.lastTaskInfo().getTimeMillis());
