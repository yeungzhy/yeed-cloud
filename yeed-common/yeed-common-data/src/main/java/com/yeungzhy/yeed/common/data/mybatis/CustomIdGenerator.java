 ...
 *
 * <p> 机器号由外部注入而非本类自选：静态配置在每个实例的 application.yml 显式指定
 * {@code mybatis-plus.global-config.worker-id} / {@code data-center-id}，
 * 动态分配则由上层用 Redis 自增下发（workerId 超 31 就 dataCenterId+1，组合出最多 32×32=1024 个实例）。
 * 无论哪种，落地形态都是把固定值传给本构造器
 *
