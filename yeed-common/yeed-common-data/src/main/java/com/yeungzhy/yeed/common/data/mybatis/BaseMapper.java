...
public interface BaseMapper<T> extends com.baomidou.mybatisplus.core.mapper.BaseMapper<T> {
     ...
     *
     * @param queryWrapper 查询条件；WHERE 自动追加 {@code @TableLogic} 未删除条件，已删记录不会被判为存在
     ...
     *
     * @param consumer 条件装配器，在函数内调用 {@code eq} / {@code like} 等方法装配查询条件
     * @return true 存在；false 不存在
     * @since 2026-08-08
     */
     ...
     *
     * @param column 实体属性的方法引用，如 {@code SysUser::getUsername}
     * @param value  待比较的值；null 的等值语义由 MyBatis-Plus 决定，唯一性校验请自行前置判空
     * @return true 存在；false 不存在
     * @since 2026-08-08
     */
     ...
     *
     * <p>Wrapper 未携带有效条件时兜底 {@code WHERE 1=0}，一条都不删，防止误删全表；
