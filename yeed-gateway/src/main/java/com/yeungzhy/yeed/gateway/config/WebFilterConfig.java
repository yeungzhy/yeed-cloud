 ...
 *
 * <p>Spring WebFlux 按 order 从小到大排序执行，数值越小越先执行；order 相同时保持注册顺序
 * <p>谁先注册谁先执行，而 {@code @Bean} 方法在配置类里按声明顺序注册。
 *
 * <p>WebFilter 层全部执行完毕后，才轮到 GlobalFilter 层（网关 {@code GatewayFilterChain} 的路由转发）；
 * 两层的 order 是各自独立排序、不能混比，GlobalFilter 的 order 再小也不会早于 WebFilter 执行。
 *
 * <p>WebFilter 执行顺序（新增时要更新本列表）：</p>
 * <ol>
 *   <li>{@link #corsWebFilter()} —— {@code Ordered.HIGHEST_PRECEDENCE} -2147483648，注册在最前</li>
 *   <li>{@link #loginUserSnapshotFilter()} —— {@code +10}</li>
 *   <li>{@link #accessLogWebFilter()} —— {@code +20}</li>
 *   <li>{@link cn.dev33.satoken.reactor.filter.SaReactorFilter} —— 源码 {@code @Order(-100)}</li>
 *   <li>其余 WebFilter</li>
 * </ol>
 *
